	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo leia a descrição pra não ser removido*${mdata.subject}*\n\nEspero que goste do grupo ❤️`
				...
				teks = `Tchau @${num.split('@')[0]} Até quando nós se encontra dnv hehe 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MINATO BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Quem é você por acaso e Minato? Comendo Recomendado Apenas pro minato❌',
					ownerB: '[❗] Quem é você por acaso e Minato? Comendo Recomendado Apenas pro minato❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Minato. Registre-se para fazer amizade com o bot Minato por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Minato|18\n\n──「 MINATO ☔ 」──`,
					admin: '[❗] Por acaso você er admin do grupo pra usa esse comando fdp❌',
					Badmin: ' [❗] Por acaso sou admin pra usa esse comando acha que sou macumbeiro pra vira admin do nada❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511934488829@s.whatsapp.net","5511934488829@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5511934488829@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5511934488829@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5511934488829@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5511934488829@s.whatsapp.net","994408606876@s.whatsapp.net","12267740582@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net","12267740582@s.whatsapp.net"]
	...
	if (messagesC.includes("adm lixo")){
      ...
      client.sendMessage(from, MessageType + " / " + messageType, MessageType.text);
       ...
       if (messagesC.includes("://chat.whatsapp.com/")){
		...
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply(`*Por falta de respeito ao admin você será removido.*`)
		client.updatePresence(from, Presence.composing)
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('tchau seu fpd próxima vez respeita o admin👋')
		}, 1100)
		setTimeout( () => {
		client.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
					}, 1000)
		setTimeout( () => {
		reply(`xingamento detectado só por causa disso será removido`)
		}, 0)
		}
...
const get = require('got')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("koe man")){
			...
			if (messagesC.includes("minato")){
			client.updatePresence(from, Presence.composing)
			reply("quer oq com meu dono gay")
	}
			...
			client.updatePresence(from, Presence.composing)
			reply("É com . caralho parece burro")
	}
		...
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("seu pai aquele boi de dois chifre sksks")
	}
		...
		if (messagesC.includes("bot tá off? ")){
			client.updatePresence(from, Presence.composing)
			reply("olha eu aqui gayzao só er usa meu comando")
	}
...
//msc fim
			...
			switch(command) {
				...
				case 'menu1':
					...
					if (!isOwner) return reply('Por acaso se chama Minato?')
...
break
		   ...
		   await limitAdd(sender)
				...
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você quem É O MINATO por acaso? ')
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO MINATO BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @minato domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Minato domina ✓*'})
                    break
                    case 'animemenu':
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MINATO ✨* 」\n*│+ wa.me/5511934488829*╰──────*「 *MINATO* 」*────*\n\n*_SE QUER SER ADMIN DO BOT MINATO_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* MINATO ☔\n*YOUTUBE:* MINATO YT\n*WPP:* wa.me/+5511934488829\n*INSTA:* OFF\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
